...
class GaussApp:
    def __init__(self, root, volver_callback):
        ...
        self.root = root
        self.root.title("Método de Eliminación de Gauss")
        ...
        self.matriz_triangular = None  # Resultado de la eliminación hacia adelante
        self.matriz_final = None       # RREF auxiliar para extraer soluciones
        self.soluciones = None
        self.tipo_solucion = None
        self.detalle_button = None
        self.mostrando_detalles = False

        # El botón "Volver" se crea dentro de frame_top (junto al de limpiar)
    ...
    # ---------------------------------------------------------
    # Convertir datos y aplicar Gauss
    # ---------------------------------------------------------
    def resolver(self):
        try:
            matriz_original = []
            ...
            pasos = self.gauss_eliminacion(A, self.filas, self.columnas)
            self.pasos_guardados = pasos
            self.matriz_triangular = deepcopy(A)

            self.matriz_final = self._rref_para_soluciones(deepcopy(A))
            self.soluciones, self.tipo_solucion = self._extraer_soluciones(self.matriz_final)
    ...
    def mostrar_resumen(self):
        ...
        self.text_result.insert(tk.END, "===== SOLUCIÓN FINAL (Gauss) =====\n\n", ("bold",))
        ...
        if self.matriz_triangular:
            self.text_result.insert(tk.END, "Matriz triangular superior obtenida (Gauss):\n", ("bold",))
            for ln in self.format_matriz_lines(self.matriz_triangular):
                self.text_result.insert(tk.END, ln + "\n")
            self.text_result.insert(tk.END, "\n")
    ...
    def mostrar_detalles(self):
        ...
        if self.matriz_triangular:
            self.text_result.insert(tk.END, "Matriz triangular superior final:\n", ("bold",))
            for ln in self.format_matriz_lines(self.matriz_triangular):
                self.text_result.insert(tk.END, ln + "\n")
            self.text_result.insert(tk.END, "\n")
        ...
        if soluciones is None:
            self.text_result.insert(tk.END, "Sistema incompatible (sin solución).\n")
        else:
            for i, val in enumerate(soluciones):
                self.text_result.insert(tk.END, f"x{i+1} = {val}\n")
        self.text_result.configure(state="disabled")
    ...
    # ---------------------------------------------------------
    # Algoritmo Gauss (triangular superior)
    # ---------------------------------------------------------
    def gauss_eliminacion(self, A, n, m):
        pasos = []
        fila_pivote = 0
    ...
    # ---------------------------------------------------------
    # RREF auxiliar para extraer soluciones (no se muestra)
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Construir representación del sistema A|b como ecuaciones
    # ---------------------------------------------------------
    ...
    def vectores_columna_lado_a_lado(self, vectores, nombres, espacio_entre_vectores=4):
        ...
        max_encabezado = max(len(e) for e in encabezados) if encabezados else 0
